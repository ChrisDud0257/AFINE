...
def main(args):
    ...
    save_txt_path = os.path.join(args.save_path, "Triplet.txt")
    ...
    for label_name in label_name_list:
        ...
        if len(group_2difference_effective_combination) > 0:
            PSY_count, PNY_count, SNY_count, SSY_count = data_statistics_2difference(*group_2difference_effective_combination)
            all_PSY_count = all_PSY_count + PSY_count
            all_PNY_count = all_PNY_count + PNY_count
            all_SNY_count = all_SNY_count + SNY_count
            all_SSY_count = all_SSY_count + SSY_count
            for combination in group_2difference_effective_combination:
                final_gt_score_12 = fetch_gt_label_for_2difference(combination[0][1], combination[1][1])
                final_gt_score_1ref = fetch_gt_label_for_1element(combination[0][1])
                final_gt_score_2ref = fetch_gt_label_for_1element(combination[1][1])
                img1 = f"{combination[0][0]}.png"
                img2 = f"{combination[1][0]}.png"
                reference_img = f"{label_name}.png"
                save_txt.write(f"{img1},{img2},{reference_img},{final_gt_score_12},{final_gt_score_1ref},{final_gt_score_2ref}\n")
        ...
        if len(group_1element_effective_combination) > 0:
            PYY_count, SYY_count, NYY_count = data_statistics_1element(*group_1element_effective_combination)
            all_PYY_count = all_PYY_count + PYY_count
            all_SYY_count = all_SYY_count + SYY_count
            all_NYY_count = all_NYY_count + NYY_count
            for combination in group_1element_effective_combination:
                final_gt_score_12 = fetch_gt_label_for_1element(combination[1])
                final_gt_score_1ref = final_gt_score_12
                final_gt_score_2ref = 0.5
                img1 = f"{combination[0]}.png"
                img2 = f"{label_name}.png"
                reference_img = f"{label_name}.png"
                save_txt.write(f"{img1},{img2},{reference_img},{final_gt_score_12},{final_gt_score_1ref},{final_gt_score_2ref}\n")
    save_txt.close()
    print(f"PSY:{all_PSY_count}, PNY:{all_PNY_count}, SNY:{all_SNY_count}, SSY:{all_SSY_count}, PYY:{all_PYY_count}, SYY:{all_SYY_count}, NYY:{all_NYY_count}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type = str, default = '/home/notebook/data/group/chendu/DiffIQA/Train/trainlabel/Triplet')
