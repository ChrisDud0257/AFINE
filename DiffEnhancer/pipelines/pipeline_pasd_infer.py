...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def prepare_latents(self, args, image, batch_size, num_channels_latents, 
                        height, width, dtype, device, generator, latents=None, init_latent_from_noise = True, added_noise_level = 200, num_inference_steps = 200):
        ...
        if init_latent_from_noise:
            print(f"Start from noise latent")
            if latents is None:
            ...
            print(f"Start from GT latent, added_noise_level is {added_noise_level}")
            ...
            added_latent_timestep = torch.LongTensor([added_noise_level]).repeat(batch_size * 1).to(self.device)
    ...
    def __call__(
        ...
        print(f"num_inference_steps is {num_inference_steps}")
        ...
        latents = self.prepare_latents(
            ...
            generator,
            latents,
            init_latent_from_noise = init_latent_from_noise,
