...
from dataloader.localmoredatasetgtinoutdegra import LocalMoreImageDatasetGTInOutDegra
...
def noise_sche(beta_start=0.00085, beta_end=0.012, num_train_timesteps=1000):
    betas = (
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        for _ in range(args.num_validation_images):
            with torch.autocast("cuda"):
                image = pipeline(
                    validation_prompt, validation_image, num_inference_steps=20, generator=generator, width=width,
                    height=height,
                ).images[0]
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--degradation_param_path",
        type=str,
        default = None
    )

    parser.add_argument('--trainable_modules', nargs='*', type=str,
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = ControlNetModel.from_unet(unet = unet, return_rgbs= False)
            ...
            for i in range(len(models)):
                ...
                if not isinstance(model, UNet2DConditionModel):
                    load_model = ControlNetModel.from_pretrained(input_dir,
                                                                 subfolder="controlnet")  # , low_cpu_mem_usage=False, ignore_mismatched_sizes=True
                else:
                    load_model = UNet2DConditionModel.from_pretrained(input_dir,
                                                                      subfolder="unet")  # , low_cpu_mem_usage=False, ignore_mismatched_sizes=True
    ...
    # Optimizer creation
    params_to_optimize = list(controlnet.parameters()) + list(unet.parameters()) #+ list(vae.decoder.parameters())
    ...
    print(f"args.gt_dir is {args.gt_dir}")
    train_dataset = LocalMoreImageDatasetGTInOutDegra(gt_dir = args.gt_dir,image_size=args.resolution, tokenizer=tokenizer, 
                                                      accelerator=accelerator, control_type=args.control_type, null_text_ratio=0.5, resize_bak=True,
                                                      opt_path=args.degradation_param_path)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet), accelerator.accumulate(unet):
                ...
                if accelerator.sync_gradients:
                    params_to_clip = list(controlnet.parameters()) + list(unet.parameters()) #+ list(vae.decoder.parameters())
