...
class RealESRGAN_degradation(object):
    def __init__(self, opt_path=None, device='cpu'):
        ...
        self.second_stage_depra_prob = self.opt['second_stage_depra_prob']

        # blur settings for the first degradation
        self.blur_kernel_size_min = optk['blur_kernel_size_min']
        self.blur_kernel_size_max = optk['blur_kernel_size_max']
        ...
        self.blur_kernel_size_min2 = optk['blur_kernel_size_min2']
        self.blur_kernel_size_max2 = optk['blur_kernel_size_max2']
        ...
        self.kernel_range1 = [2 * v + 1 for v in range(self.blur_kernel_size_min, self.blur_kernel_size_max + 1)]  # kernel size
        self.pulse_tensor = torch.zeros(self.kernel_range1[-1], self.kernel_range1[-1]).float()  # convolving with pulse tensor brings no blurry effect
        self.pulse_tensor[self.kernel_range1[-1] // 2, self.kernel_range1[-1] // 2] = 1

        self.kernel_range2 = [2 * v + 1 for v in range(self.blur_kernel_size_min2, self.blur_kernel_size_max2 + 1)]  # kernel size
    ...
    def random_augment(self, img_gt):
        ...
        img_gt, status = augment(img_gt, hflip=True, rotation=True, return_status=True)
        """
        # random color jitter 
    ...
    def random_kernels(self):
        ...
        if np.random.uniform() < self.sinc_prob:
            # this sinc filter setting is for kernels ranging from [3, 9]
            if kernel_size < 9:
            ...
            kernel = random_mixed_kernels(
                    ...
                    noise_range=None)
        # pad kernel
        pad_size = (self.kernel_range1[-1] - kernel_size) // 2
        ...
        if np.random.uniform() < self.final_sinc_prob:
            kernel_size = random.choice(self.kernel_range2)
            omega_c = np.random.uniform(np.pi / 3, np.pi)
            sinc_kernel = circular_lowpass_kernel(omega_c, kernel_size, pad_to=self.kernel_range1[-1])
    ...
    def degrade_process(self, img_gt, resize_bak=False):
        ...
        scale_final = np.random.uniform(self.scale[0], self.scale[1])
        ...
        if random.random() < self.second_stage_depra_prob:
        ...
        if np.random.uniform() < 0.5:
            ...
            mode = random.choice(['area', 'bilinear', 'bicubic'])
            out = F.interpolate(out, size=(int(ori_h // scale_final), int(ori_w // scale_final)), mode=mode)
        ...
        mode = random.choice(['area', 'bilinear', 'bicubic'])
        img_gt_only_downsample = F.interpolate(img_gt, size=(int(ori_h // scale_final), int(ori_w // scale_final)), mode=mode)
        img_gt_only_downsample = F.interpolate(img_gt_only_downsample, size=(ori_h, ori_w), mode=mode)

        return img_gt, img_lq, img_gt_only_downsample
